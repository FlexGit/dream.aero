		...
		<div class="col-8">
			<div class="form-group">
				@if($deal->contractor->email == app('\App\Models\Contractor')::ANONYM_EMAIL || $user->isSuperAdmin())
					...
					{{--<div class="d-flex">--}}
						<input type="text" class="form-control" id="contractor_search" name="contractor_search" value="{{ $deal->contractor ? $deal->contractor->email : '' }}" placeholder="{{--Email or Phone number--}}" {{ $deal->contractor ? 'disabled' : '' }}>
	...
	@if($deal->product && $deal->product->productType && $deal->product->productType->alias == app('\App\Models\ProductType')::TAX_ALIAS)
		<div class="col-4">
			<div class="form-group">
				...
				<input type="text" class="form-control" id="product_amount" name="product_amount" value="{{ $deal->amount }}" placeholder="">
			</div>
		</div>
	@endif
